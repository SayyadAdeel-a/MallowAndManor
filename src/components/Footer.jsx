...
export default function Footer() {
  ...
  useEffect(() => {
    fetchSettings().then(s => setFooter(s?.footer)).catch(() => {});
  }, []);
    ...
    <footer className="bg-brand-wine-dark text-brand-cream border-t border-brand-burgundy">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex gap-4">
              ...
              <a href={f.tiktok || "https://www.tiktok.com/@honeybeelane?lang=en"} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 border border-brand-gold/30 rounded-full flex items-center justify-center text-brand-gold hover:bg-brand-gold hover:text-brand-wine-dark transition-all" aria-label="TikTok">
                <AnimatedIcon path={ICONS.tiktok} animation="bounce" className="w-4 h-4" fill="currentColor" strokeWidth={0} />
              </a>
            </div>
          </div>
        ...
        <div className="border-t border-brand-burgundy pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-brand-cream/50">
          <p>&copy; 2026 {f.copyrightText || "Honeybee Lane. All rights reserved."}</p>
          <div className="flex items-center gap-6">
            <a href="https://syab.tech/mentee" target="_blank" rel="noopener noreferrer" className="hover:text-brand-gold transition-colors">
