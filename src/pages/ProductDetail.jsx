  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      ...
      fetchProductById(id)
        .then(data => {
          if (data && !data.error) {
            const mapped = { id: data._id, slug: data.slug, name: data.name, price: data.price, category: data.category, mainImage: data.mainImage, thumbnails: data.thumbnails || [], description: data.description, highlights: data.highlights || [], createdAt: data.createdAt };
            setProduct(mapped);
    ...
    (async () => {
      try {
        ...
        const list = (Array.isArray(data) ? data : data?.products || [])
          ...
          .slice(0, 8)
          .map(p => ({ id: p._id, slug: p.slug, name: p.name, price: p.price, category: p.category, mainImage: p.mainImage, thumbnails: p.thumbnails || [], description: p.description }));
        setRelated(list);
      } catch { /* silent */ }
    })();
